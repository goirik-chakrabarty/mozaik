...
def _sheet_segs():
    ...
    L4, L23 = "V1_Exc_L4", "V1_Exc_L2/3"  # canonical order puts L4 (prio 2) before L23 (prio 4)
    return [
        # presentation 0: imgA (dur 100)
        _Seg({"trial": 0, "movie_name": "imgA", "duration": 100},
             [np.array([10.0, 50.0, 150.0]), np.array([20.0])], sheet_name=L4),   # 150 >= dur dropped
        _Seg({"trial": 0, "movie_name": "imgA", "duration": 100},
             [np.array([5.0]), np.array([]), np.array([7.0])], sheet_name=L23),
        ...
        _Seg({"trial": 0, "movie_name": "imgC", "duration": 80},
             [np.array([5.0, 79.0]), np.array([])], sheet_name=L4),
        _Seg({"trial": 0, "movie_name": "imgC", "duration": 80},
             [np.array([1.0]), np.array([2.0]), np.array([80.0])], sheet_name=L23),  # 80 >= dur dropped
    ]


def test_multi_sheet_stacks_units_and_records_boundaries(tmp_path):
    out = str(tmp_path / "responses")
    exp = MozaikTrialExporter(out, trial_id=0, sampling_rate=1000.0)  # sheet_names=None -> all
    ...
    assert meta["end_time"] == 0.230
    assert meta["stimuli_order"] == ["imgA", "blank", "imgC"]      # one entry per presentation
    ...
    np.testing.assert_allclose(units[2], np.array([5.0, 151.0]) / 1000.0)    # imgA[5]@0 + imgC[1]@150
    np.testing.assert_allclose(units[3], np.array([152.0]) / 1000.0)         # imgC[2]@150
    np.testing.assert_allclose(units[4], np.array([7.0]) / 1000.0)           # imgA[7]@0 (imgC[80] dropped)


def test_multi_sheet_subset_selection(tmp_path):
    out = str(tmp_path / "responses")
    exp = MozaikTrialExporter(out, trial_id=0, sampling_rate=1000.0,
                              sheet_names=["V1_Exc_L2/3"])
    exp.process_batch([_DSV(_sheet_segs())])
    exp.finalize()
    spikes, meta = _load(out)
...
def test_multi_sheet_unequal_counts_raise(tmp_path):
    ...
    segs = [
        _Seg({"trial": 0, "movie_name": "imgA", "duration": 100}, [np.array([1.0])], sheet_name="V1_Exc_L4"),
        _Seg({"trial": 0, "movie_name": "imgC", "duration": 80}, [np.array([2.0])], sheet_name="V1_Exc_L4"),
        _Seg({"trial": 0, "movie_name": "imgA", "duration": 100}, [np.array([3.0])], sheet_name="V1_Exc_L2/3"),
    ]
